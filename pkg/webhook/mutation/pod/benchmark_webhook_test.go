			...
			_, err := client.CoreV1().Namespaces().Create(context.TODO(), &corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					...
					Labels: map[string]string{
						...
						"true":       "true",
						"false":      "false",
						"dynakube.internal.dynatrace.com/instance": testDkPrefix + strconv.Itoa(cnt%dkCnt),
					},
				},
...
func createDynakubes() {
	...
	for i := 0; i < dkCnt; i++ {
		dk := createTestDynakube(testDkPrefix + strconv.Itoa(i)).WithOneAgentNamespaceSelector(metav1.LabelSelector{
